...
Kq=rigidez_solo.F(Xq,Zq);
disc_rig=[max(Kq)/10*.9999:max(Kq)/10*.9999:max(Kq)*.9999];
cnt=contourc(Xq(1,:),Zq(:,1),Kq,disc_rig);
...
for k = 1:contourNo-1
    ...
    text(contourXY{k}(1,end),y,contourXY{k}(2,end),['   ' num2str(labelcnt(k)/1E6,3) ' MN/m'])
end
