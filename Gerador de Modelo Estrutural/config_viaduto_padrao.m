...
viaduto.Ti=20;%Temperatura média diária do ambiente em graus Celsius
viaduto.nivel_minimo_de_protensao=2;% 2 é protensão limitada, 3 é protensão completa, protensão parcial não é aceito.
viaduto.DMLG=10;%(Data da montagem das longarinas) em dias, usado na instabilidade da longarina e no calculo da fluencia dos pilares
viaduto.DCLJ=15;%(Data da concretagem da laje) em dias, usado na instabilidade da longarina e no dimensionamento a flexão da longarina
...
viaduto.folga=.35;%m Folga entre nó inicial e final das longarinas consecutivas 
...
viaduto.dia_max_agr_grau=.025;% Diamensão máxima do agregado graúdo em metros
%Brita 1, 12,5 mm
%Brita 2, 25,0 mm
%Brita 3, 38,5 mm
...
viaduto.danc=.066*2;% distancia entre eixo de ancoragens de protensão
...
viaduto.dimensionar_barras_fustes_individualmente=false; %dimensionar todas as barras dos fustes = true; uma vez para o maior esforço das barras dos fustes = false;
...
viaduto.pap.himax=.016;%espessura maxima da camada elastomero em metros
viaduto.pap.fb=.1;%diferença entre a largura da base da longarina e a largura do aparelho de apoio em metros
viaduto.pap.a_sobre_b=.6;%relação entre a menor dimensão sobre a maior dimensão
