...
fprintf([datestr(now,'HH:MM:SS') ': SPEA2 v1.1.\n']);
% v1.1 - Preferencia dobrada para soluções de borda.
...
parametros_SPEA2.pop_size = 100; % Tamanho da população
parametros_SPEA2.maxgen = 10000; % Numero máximo de iterações
parametros_SPEA2.p_c = .9; % Probabilidade de cruzamento
parametros_SPEA2.p_m = .25; % Probabilidade de mutação - recomendado 1/numero_variaveis %.2 % .2 %.5
parametros_SPEA2.eta_c = 5; % SBX - Operador de cruzamento % 20 % 40 % 5
parametros_SPEA2.eta_m = 10; % Polynomial mutation operator % 2  % 60 % 10


%% Inicialização da população
pop_size = parametros_SPEA2.pop_size;
maxgen = parametros_SPEA2.maxgen;
p_c = parametros_SPEA2.p_c;
p_m = parametros_SPEA2.p_m;
eta_c = parametros_SPEA2.eta_c;
eta_m = parametros_SPEA2.eta_m;
...
R = calc_strength(fitness, min_or_max);


%% k-th Nearest Neibor Method for Diversity

D = calc_density(fitness);
...
while t <= maxgen
    ...
    M = crowded_binary_tournament_selection(R, -D);
    ...
    is = zeros(1,n_sucessos); % is = indice dos individuos que teveram sucesso
    n_sucessos = 0;
    for i = 1 : length(Q)
        if Q{i}.violations == 0
            n_sucessos = n_sucessos + 1;
    ...
    R = calc_strength(total_fitness, min_or_max);


    %% k-th Nearest Neibor Method for Diversity

    D = calc_density(total_fitness);


    %% Cria Frontes F ficticios baseados em Strength

    F = cria_F_baseado_strength(R, D); % F sai organizado por D, os primeiros dentro do mesmo F possuem os menores D
    ...
    for f = 1:length(F)
        if (n_new_pop + length(F{f})) <= pop_size
            ...
            if f == 1
                clear fitness_deletada
                for i = 1:length(F{f})-pop_size
                    F_fitness = total_fitness(F{f},:);
                    iremove = truncamento(F_fitness);
                    fitness_deletada(i,:) = F_fitness(iremove,:);
                    F{f}(iremove) = [];
                end
            end
  ...
  %
     ...
     %        if exist('fitness_deletada','var')
     %            scatter3(fitness_deletada(:,1),fitness_deletada(:,2),fitness_deletada(:,3),'x')
     %            clear fitness_deletada
     %        end
     %        grid on
     %        hold off
  ...
  %
    ...
    save([mcrdir 'saida_' algoritimo] ,'parametros_SPEA2','tempo_analise','t','populacao','hypervolume', 'pop_fitness')
